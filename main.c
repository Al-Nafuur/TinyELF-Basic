...
#define LEFT74_14            0x60
#define LEFT74_13            0x50
#define LEFT74_12            0x40
#define LEFT74_11            0x30
#define LEFT74_10            0x20
#define LEFT74_9             0x10
#define LEFT74_8             0x00
#define LEFT74_7             0xF0
#define LEFT74_6             0xE0
#define LEFT74_5             0xD0
#define LEFT74_4             0xC0
#define LEFT74_3             0xB0
#define LEFT74_2             0xA0
#define LEFT74_1             0x90
#define NO_MO_74             0x80
...
// NUSIZx
#define ONE_COPY             0x00
#define TWO_COPIES_CLOSE     0x01
#define TWO_COPIES_MED       0x02
#define THREE_COPIES_CLOSE   0x03
#define TWO_COPIES_WIDE      0x04
#define DOUBLE_SIZE          0x05
#define THREE_COPIES_MED     0x06
#define QUAD_SIZE            0x07

// CTRLPF bits
#define PF_REFLECT           0x01
#define PF_SCORE             0x02
#define PF_PRIORITY          0x04
#define BALL_8_CLKS          0x30
#define BALL_4_CLKS          0x20
...
#define P1_POSITION              64
#define M0_POS                  143
#define BORDER_SHAPE           0x30
#define USE_ENAM0                 0
#define USE_ENAM1                 1
...
RAM_FUNC void KERNEL_1_line(const uint8_t *linePointer, uint8_t flag12) {
    ...
    if (flag12 == 0){
       	vcsJmp3();  // SLEEP(5)
        vcsNop2();
        vcsLda2(linePointer[1]); // lda #{1} -> R?L??_03_04  (index 1)
        vcsSta3(GRP0);
    } else {
        vcsLdx2(ENAM1);
        vcsTxs2();
        vcsLda2(linePointer[1]);
        // sta GRP0-ENAM1,X -> STA zp,X (opcode 0x95) where zp = GRP0 - ENAM1
        vcsSta4(GRP0);
    }

    // ldx #LEFT74_8 | {11} where {11} is next-line BIT0_ at linePointer[39]
    vcsLdx2((uint8_t)(LEFT74_8 | linePointer[39]));
    vcsLdy2(linePointer[10]); // ldy #{5} -> R?L??_21_22 (index 10)
    vcsLda2(linePointer[8]);  // lda #{4} -> R?L??_17_18 (index 8)
    vcsSta3(GRP1);
    vcsSta3(RESP0);
    vcsLda2(linePointer[3]);  // lda #{2} -> R?L??_07_08 (index 3)
    vcsSta3(GRP0);
    vcsLda2(linePointer[5]);  // lda #{3} -> R?L??_11_12 (index 5)
    vcsSta3(GRP0);
    vcsStx3(HMP1);
    vcsLda2(linePointer[13]); // lda #{7} -> R?L??_27_28 (index 13)
    vcsSta3(GRP0);
    vcsLda2(linePointer[12]); // lda #{6} -> R?L??_25_26 (index 12)
    vcsSty3(GRP1);
    vcsSta3(RESP0);
    vcsSta3(GRP1);
    vcsSta3(RESP0);
    vcsLda2(linePointer[15]); // lda #{8} -> R?L??_31_32 (index 15)
    // sta GRP0-LEFT74_8-{11},X  -> STA zp,X where base = GRP0 - LEFT74_8 - linePointer[21]
    vcsSta4(GRP0);

    vcsLda2(linePointer[17]); // lda #{9} -> R?L??_35_36 (index 17)
    vcsSta3(GRP0);
    vcsLda2(linePointer[20]); // lda #{10} -> next-line 01_02 at linePointer[20]
    vcsSta3(HMOVE);
}

RAM_FUNC void KERNEL_2_line(const uint8_t *linePointer, uint8_t use_enam, uint8_t colubk) {
    vcsSta3(RESP0);
    vcsSta3(GRP0);
    vcsStx3((uint8_t)(ENAM0 + use_enam)); // stx ENAM0+{10} -> use_enam used as small offset
    vcsLda2(linePointer[7]); // lda #{4} -> R?L??_05_06 (index 2)
    vcsSta3(GRP1);
    vcsLdy2(linePointer[2]); // ldy #{1} -> R?L??_09_10 (index 4)
#if FREE_CYCLES_AT_KERNEL_END == 1
    vcsLdx2((uint8_t)(RIGHT_8));
    vcsLda2(linePointer[18]); // lda #{9} -> R?L??_33_34 (index 16)
    vcsStx3(HMP1);
#else
    vcsLda2(colubk);
    vcsSta3(COLUBK);
    ...
    vcsLda2(linePointer[18]); // lda #{9} -> R?L??_33_34 (index 16)
#endif
    vcsLdx2(linePointer[4]);  // ldx #{2} -> R?L??_13_14 (index 6)
    vcsSta3(GRP0);
    vcsSta3(RESP0);
    vcsSty3(GRP0);
    vcsStx3(GRP0);
    vcsLda2(linePointer[6]);  // lda #{3} -> R?L??_15_16 (index 7)
    vcsSta3(GRP0);
    vcsLdx2(linePointer[14]); // ldx #{7} -> R?L??_29_30 (index 14)
    vcsLda2(linePointer[9]);  // lda #{5} -> R?L??_19_20 (index 9)
    vcsSta3(GRP1);
    vcsLda2(linePointer[11]); // lda #{6} -> R?L??_23_24 (index 11)
    vcsSta3(GRP1);
    vcsSta3(RESP0);
    vcsStx3(GRP0);
    vcsSta3(RESP0);
    vcsLda2(linePointer[16]); // lda #{8} -> R?L??_33_34 (index 16)  (also used above as {9})
    vcsSta3(GRP0);
    // IF {8} == 0 -> pha else php
    if (linePointer[16] == 0) vcsPha3();
    else vcsPhp3();

    // end-of-kernel: choose based on free_cycles_at_end
#if FREE_CYCLES_AT_KERNEL_END == 1
    vcsSleep(5);
#else
    vcsLdx2(RIGHT_8);  // ldx #RIGHT_8 ; stx HMP1
    vcsStx3(HMP1);
#endif
}
...
RAM_FUNC void renderDisplayFrame(uint8_t evenFrame) {
    ...
    vcsSta3(COLUP0);
//        vcsLda2(colRow_1); // we use same color for P0 and P1
    vcsSta3(COLUP1);

    vcsLda2(COL_BORDER);
    vcsSta3(COLUPF);
    vcsLda2(BALL_1_CLK | PF_PRIORITY | PF_REFLECT);
    vcsSta3(CTRLPF);

    vcsLda2(BORDER_SHAPE);
    vcsSta3(PF0);
    vcsLda2(THREE_COPIES_CLOSE);
    vcsSta3(NUSIZ0);
    vcsSta3(NUSIZ1);

    vcsLda2(LEFT74_14);
    vcsSta3(HMP0);
    vcsLda2(LEFT74_4);   // LEFT74_4 and RIGHT_4
    vcsSta3(HMM0);
    vcsSta3(HMM1);
    vcsLda2(RIGHT_8);
    vcsSta3(HMP1);
    ...
    if (evenFrame){
        ...
        vcsLda2(linePtr[0]); //    #R{1}L01_01_02              ;2
        vcsLdx2(LEFT74_8 | linePtr[19]); //#LEFT74_8 | R{1}L01_BIT0_   ;2
        vcsSta3(HMOVE);
    }
    ...
    for (uint8_t row = 0; row < NUM_OF_ROWS; row++) {
        if (kernel_a){ // KERNEL_A
            ...
            // KERNEL_A_END
            vcsSta3(RESP0); //3  @1
            vcsSta3(HMOVE); //3  @4    1 cycle late
            vcsLda2(0);     //2  @6
            vcsSta3(GRP0);  //3  @9
            vcsSta3(GRP1);  //3  @12
